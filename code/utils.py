...
# ---------------------------------------------------------------------------
# Class taxonomy - 3 classes, ordered by how much is on the plate (low -> high).
# The order is fixed so label indices and confusion-matrix axes are stable across
# notebooks. The index of each class is its position in this list.
# ---------------------------------------------------------------------------
CLASS_NAMES = [
    "clean",     # 0 - pristine, unused plate (no food/crumbs)          -> do not clear
    "finished",  # 1 - meal is over: eaten bare OR only small leftovers  -> clear
    "full",      # 2 - a moderate-to-full serving of food                -> do not clear
]

# Every class has its own prompts (all three are generated by build_prompts()).
CONTENT_CLASSES = CLASS_NAMES
CLASS_TO_IDX = {name: i for i, name in enumerate(CLASS_NAMES)}
IDX_TO_CLASS = {i: name for i, name in enumerate(CLASS_NAMES)}

# Binary decision rule (NON-MONOTONIC by design): a `clean` plate has the least on
# it yet maps to "do not clear" (freshly set), while `finished` in the middle maps
# to "clear" and `full` (the most food) maps back to "do not clear".
CLEAR_CLASSES = {"finished"}
DONT_CLEAR_CLASSES = {"clean", "full"}


def to_binary(class_name: str) -> str:
    """Map a class to an action: 'clear' or 'do_not_clear'."""
...
# ---------------------------------------------------------------------------
# Project paths.  LAYOUT: this file lives in  <Project Main Folder>/code/ , with
# sibling folders  <Project Main Folder>/data/  and  <Project Main Folder>/weights/ .
#   *** THIS BLOCK IS THE ONLY PLACE TO ADJUST PATHS IF YOU MOVE THINGS. ***
#   - Keep code / data / weights as siblings  -> nothing to change.
#   - If `data` is a CHILD of `code` again     -> use  ROOT_DIR / "data".
#   - On a GPU box, point at data without edits -> set the PLATE_DATA_ROOT env var.
# ---------------------------------------------------------------------------
ROOT_DIR = Path(__file__).resolve().parent              # <Project Main Folder>/code/  (utils.py lives here)
PROJECT_DIR = ROOT_DIR.parent                           # <Project Main Folder>/
DATA_DIR = Path(os.environ.get("PLATE_DATA_ROOT", PROJECT_DIR / "data"))   # sibling data/ (env-overridable)
...
RESULTS_DIR = PROJECT_DIR / "weights"                   # trained model weights (.pth) live in sibling weights/
...
CUTLERY_STATES = [
    ...
    # (`finished`) via USED_NAPKINS below, never to clean/full.
]
LIGHTINGS = [
...
# Leftover-friendly foods for the `finished` small-leftover phrasings. Error analysis showed
# solid / composite plated meals (steak & mash, salmon & asparagus, stir-fry & rice) get drawn
# as FULL servings - you can't render "a tiny smear of a steak". Saucy / amorphous / scrappy
# foods render believably as a small remnant, so the {food} slots use only these.
LEFTOVER_FOODS = [
    # pasta sauces
...
CLASS_CONTENTS = {
    ...
    # `finished` = the meal is OVER -> clear. Covers the whole range: eaten bare (only
    # crumbs / sauce smears / residue, no food) AND small leftovers (a tiny smear or a few
    # scraps of {food}). Phrasings lead with EMPTINESS + "smear"/"scraps" so FLUX draws a
    # small remnant, not a real portion ("a little leftover {food} pushed to one side" made
    # FLUX over-fill, so it was removed). {food} is filled from LEFTOVER_FOODS (saucy /
    # amorphous foods that render believably as a small remnant).
    "finished": [
        # -- eaten bare: only crumbs / smears / residue, no real food --
        ...
        # -- small leftovers: just a tiny bit of {food} left --
    ...
    # `full` spans MODERATE -> full and is "do not clear". Mix complete portions with
    # meals-in-progress that still have PLENTY of food left (clearly fuller than finished).
    "full": [
...
# Appended to ~20% of `finished` prompts (plates someone has eaten from); NEVER on
# `clean` (pristine) or `full` (still being eaten).
NAPKIN_CLASSES = {"finished"}
NAPKIN_PROB = 0.20
USED_NAPKINS = [
...
def _make_contents(rng: random.Random, class_name: str) -> str:
    ...
    # `finished` small-leftover phrasings use the curated smearable LEFTOVER_FOODS; `full` uses FOOD_ITEMS.
    foods = LEFTOVER_FOODS if class_name == "finished" else FOOD_ITEMS
    if "{food}" in phrasing:
        phrasing = phrasing.format(food=rng.choice(foods))
    # ~20% of `finished` (eaten-from) plates get a used napkin (a "done eating" cue).
